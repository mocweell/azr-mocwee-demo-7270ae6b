    ...
    <section id="armada" className="py-20 sm:py-28 bg-background">
      <div className="max-w-7xl mx-auto px-5 sm:px-6">
        <div className="text-center max-w-3xl mx-auto mb-12">
          ...
          <div className="mt-7 flex flex-wrap justify-center gap-2 sm:gap-3">
            {includes.map((i) => (
              <span
                key={i}
                className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-secondary text-secondary-foreground text-xs sm:text-sm font-medium"
              >
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5 sm:gap-6">
          {allInCars.map((car) => (
            <CarCard key={car.name} car={car} accent="navy" />
          ))}
        </div>
      </div>
    </section>
